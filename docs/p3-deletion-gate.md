...
| Unused challenge props | FRONTEND CLEANUP | `CheckWorkspace` no longer renders `InsightRail`. Any remaining unused frontend props can be removed after `pnpm typecheck` and focused frontend tests. | Do not touch backend challenge contracts while removing frontend-only props. |
