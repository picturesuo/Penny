...
test("self-iteration loop surfaces derived effects and feeds them into the next challenge lens", async () => {
  const targetClaimId = uuidAt(101);
  const edgeId = uuidAt(401);
  const derivedEffect = {
    id: uuidAt(801),
    kind: "shape_candidate" as const,
    ...
    payload: {
      ...
      evidenceMoveIds: [uuidAt(501)],
    },
    createdAt: "2026-04-29T12:00:00.000Z",
  };
  const response = await handleChallengeRespondRequest(
    request("http://localhost/brain/challenge/respond", {
      challengeEdgeId: edgeId,
      response: "revise",
      revisedText: "Penny should use prior critique outcomes when choosing the next challenge.",
      reasoning: "The next critique should account for what changed.",
    }),
    {
      async persistResponse(parsed) {
        return {
          ...
          targetClaim: claim(
            ...
            uuidAt(203),
            "assumption",
            "Penny should use prior critique outcomes when choosing the next challenge.",
          ),
          critiqueClaimId: uuidAt(301),
          challengeEdge: {
            id: edgeId,
  ...
  const generateText: ChallengeGenerateText = async (request) => {
    ...
    return {
      output: {
        critique: "This next critique should account for the recorded revision pattern before adding a new objection.",
        failureType: "shaky_assumption",
        strength: "moderate",
        provenanceTag: "penny:challenge.lens",
        whyThisCritique: "The pending effect shows a revision under pressure, which should shape the next challenge.",
        whatWouldResolveIt: "Show that the next challenge selected a different weakness because of the prior move.",
        suggestedNextMove: "Defend, Revise, or Absorb.",
      },
    };
  };

  await generateChallengeOutput(
    {
      targetClaimId,
      targetKind: "assumption",
      targetText: "Penny should use prior critique outcomes when choosing the next challenge.",
      ...
      lensSnapshot: {
        pendingEffects: [
          {
            id: derivedEffect.id,
            kind: derivedEffect.kind,
            title: derivedEffect.title,
            summary: derivedEffect.summary,
            payload: derivedEffect.payload,
          },
        ],
      },
    },
  ...
  assert.equal(response.status, 200);
  assert.equal(responsePayload.data.response, "revise");
  assert.equal(responsePayload.data.derivedEffects?.[0]?.kind, "shape_candidate");
  assert.equal(responsePayload.data.derivedEffects?.[0]?.status, "pending_review");
  assert.equal(responsePayload.data.derivedEffects?.[0]?.title, "Revision after pressure");
  assert.equal(calls.length, 1);
  assert.match(calls[0]?.prompt ?? "", /Lens snapshot JSON/);
  assert.match(calls[0]?.prompt ?? "", /shape_candidate/);
  assert.match(calls[0]?.prompt ?? "", /Revision after pressure/);
  assert.match(calls[0]?.prompt ?? "", /evidenceMoveIds/);
});
