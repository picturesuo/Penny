...
- May 24, 2026 research refresh: Understand Anything's public repo (`https://github.com/Lum1104/Understand-Anything`) frames the useful loop as "Scan -> Map -> Teach" for code/knowledge graphs, and Google's NotebookLM Mind Maps help (`https://support.google.com/notebooklm/answer/16212283`) emphasizes source-grounded maps whose nodes can become question targets. Penny's Learn should keep adapting that into Source -> Map -> Teach -> Use -> Check for any user-provided information, with source honesty and compact first-screen text.
- May 25, 2026 tutor research refresh: Khanmigo emphasizes tailored support plus critical thinking, Google Learn About uses learning cards like common misconceptions/stop-and-think/test-your-knowledge, Duolingo Explain My Answer gives in-the-moment personalized mistake feedback, Synthesis uses micro-assessments before advancement, and Harvard's custom AI tutor results point to self-paced feedback. Penny Learn should therefore split every concept into tiny Meaning -> Move -> Check steps, keep Ask scoped to the current step, and advance only after a small local check rather than presenting a long AI explanation.
...
- Learn map nodes should be actionable question targets: clicking Source, Map, Teach, Use, or Check should open the learner's next grounded question rather than adding more visible explanatory copy.
...
### 2026-05-25: AI Memory, Chat Archive, And Thinking Workspace Adjacent Products
- Source: user request for online research into products similar to Penny's desired brain loop, aesthetic idea docs, imported conversations, meta-analysis of thinking style, and low-busy UI.
- ChatGPT patterns to learn from: Projects group chats, uploaded files, custom instructions, and project memory inside a long-running workspace; existing chats can be moved into a project. ChatGPT memory now separates explicit saved memories from reference chat history, and OpenAI documents that chat history does not retain every detail. Pulse is the strongest adjacent pattern for Penny's background loop: once-a-day asynchronous research from past chats, memory, feedback, and connected apps, delivered as scan-friendly visual summaries. Sources: https://help.openai.com/en/articles/10169521-chatgpt-projects, https://help.openai.com/en/articles/11146739-how-does-reference-saved-memories-work, https://help.openai.com/en/articles/12293630-chatgpt-pulse.
- Claude patterns to learn from: Projects are self-contained workspaces with their own chat histories and knowledge bases; Project Knowledge can use RAG automatically as content grows. Claude can search previous chats with RAG-style tool calls, creates memory summaries updated every 24 hours, keeps separate project memory, and shows past-chat citations. Artifacts are a strong artifact model: self-contained documents/code/diagrams shown outside the main chat, with versions and export, but edits do not silently rewrite the original conversation. Sources: https://support.claude.com/en/articles/9517075-what-are-projects, https://support.claude.com/en/articles/11473015-retrieval-augmented-generation-rag-for-projects, https://support.claude.com/en/articles/11817273-use-claude-s-chat-search-and-memory-to-build-on-previous-context, https://support.claude.com/en/articles/9487310-what-are-artifacts-and-how-do-i-use-them.
- GitHub/Codex/agent patterns to learn from: GitHub Copilot Spaces centralize task context to ground Copilot responses; Copilot SDK session persistence stores resumable checkpoints, conversation snapshots, `plan.md`, and session artifacts; Codex cloud is framed as background task delegation against GitHub repositories. Claude Code's auto memory uses a concise startup `MEMORY.md` plus separate topic files, all plain Markdown and auditable by the user. Sources: https://docs.github.com/en/copilot/concepts/context/spaces, https://docs.github.com/en/copilot/how-tos/copilot-sdk/use-copilot-sdk/session-persistence, https://developers.openai.com/codex/cloud, https://code.claude.com/docs/en/memory.
- Chat archive and memory-layer competitors: Basic Memory imports ChatGPT/Claude histories and converts conversations into linked, searchable Markdown with local Markdown sync. ChatCollector exports conversations from many AI platforms to Markdown or Notion. Lore is a self-hosted encrypted AI conversation archive with ZIP imports, local search, and proper rendering of code/tool calls. Memoryport and TideMind position themselves as local-first, cross-tool memory layers; TideMind is especially close to the "living brain" language with graph recall, decay, divergent scanning, and profile injection at conversation start. Sources: https://basicmemory.com/import/chatgpt/, https://basicmemory.com/blog/local-sync-import/, https://www.chatcollector.com/, https://loreapp.dev/, https://memoryport.ai/, https://tidemind.ai/.
- Knowledge-workspace patterns to watch: Obsidian gives local Markdown, backlinks, graph view, Canvas, and broad imports; Heptabase combines note cards, whiteboards, journal capture, semantic search, and MCP tools that can read/search/write its knowledge base; Tana's Supertags and Capacities' object types show how freeform notes can become typed objects without forcing every thought into folders. Sources: https://help.obsidian.md/import, https://help.obsidian.md/plugins/canvas, https://help.obsidian.md/plugins/graph, https://support.heptabase.com/en/articles/12679581-how-to-use-heptabase-mcp, https://tana.inc/docs/tana-ai, https://docs.capacities.io/reference/content-types.
- Product implication for Penny: the closest existing products either preserve conversations, search notes, or personalize assistants, but few make critique, claim evolution, confidence, and user override history the central object. Penny should keep its moat in structured Claims, Moves, Edges, SourceSpans, sessions, and challenge artifacts rather than becoming a general archive.
- Import implication: conversation import is valuable, but should come after the first loop works. When added, imports should become SourceSpans, imported Session/Moves, extracted Claims, and optional aesthetic compiled docs. Do not import as a flat pile of chats or a generic searchable vault.
- Aesthetic docs implication: use "little docs" as compiled Idea/Claim pages with provenance, confidence, contradiction history, and related Moves. The doc can be beautiful, but it is not source of truth; it is a WikiPage or Artifact compiled from the graph.
- Background loop implication: copy the Pulse/Claude memory cadence, not the feed addiction. Run bounded reflection passes such as session close, daily review, or weekly review that produce small cards: "stale foundation," "pattern in your objections," "old idea resurfacing," "similar prior challenge," and "concept you seem ready to learn." Let the user save, ignore, or turn these into Moves.
- Meta-analysis implication: Penny's "how I think" layer should be an explicit Lens/Profile Review artifact derived from repeated moves, not hidden personalization. Track patterns like preferred reasoning mode, recurring assumptions, common deflections, favorite evidence types, challenge response style, stale foundations, and domains where confidence outruns support.
- UI implication: avoid making the sidebar the product. Prefer a calm primary workspace with structure first, plus optional progressive surfaces: a collapsible right inspector for provenance/history, a command palette for navigation/import/search, a small "review tray" for background cards, and a map/document toggle for zooming between topology and polished artifacts.
- MVP caution: the market is crowded around AI memory and second brains. Penny should borrow memory portability, import discipline, citations, and artifact ergonomics, but defer broad ingestion, browser extensions, MCP surfaces, and semantic memory until seed -> assumptions -> challenge -> defend/revise/absorb -> learn -> Idea Map + Challenge Brief is undeniable.

### 2026-05-25: Brain Mode Action Button Model
- Source: user feedback that Brain mode feels underwhelming and crowded, plus follow-up research into Lore and GitHub/open-source AI memory systems.
- Current Penny state: Brain already has many actions, but they are scattered across quick notes, search, document creation, memory import, profile review, memory review, Create handoff, and coding prompt export. The issue is action architecture, not simply button count.
- Lore lesson: Lore stays simple by centering import, browse/search, proper rendering, encryption, and backup/export. Its useful pattern is one obvious import/search archive surface, not a pile of tools. Source: https://loreapp.dev/.
- GitHub/open-source lesson: CTK exposes a small command surface for import/export/query plus a full TUI for deeper conversation operations; COG packages repeated thinking work as named skills like daily braindump, weekly reflection, monthly synthesis, strategic research, and PRD generation; Hippo adds cross-tool import, dry-run capture, decay/consolidation, active snapshots, and session trails. Sources: https://github.com/queelius/ctk, https://github.com/huytieu/COG-second-brain, https://github.com/kitfunso/hippo-memory.
- Recommended Brain buttons should be verbs from Penny's thinking loop, not generic vault controls. Candidate primary verbs: Capture, Relate, Challenge, Verify, Distill, Revive, Learn, and Use.
- Capture means drop a raw note, import chat history, or add source text. Existing support: quick note create and Brain import.
- Relate means connect this note/document/claim to another claim as supports, depends_on, contradicts, teaches, or echoes. Needed support: object-level action that opens a link picker and creates a typed Edge/Move.
- Challenge means find the weakest assumption or generate a counterargument against the selected idea. Existing support is close in Challenge/Verify routes, but Brain needs a one-click selected-object entry point.
- Verify means look for evidence and update confidence visibly. Existing support: verify route and confidence update concepts; Brain needs a selected claim/document "Verify" button.
- Distill means compile the selected thought cluster into a beautiful little Idea/Claim page. Existing support: Brain document rendering and WikiPage/Artifact doctrine.
- Revive means find old similar ideas, imported conversations, abandoned notes, or recurring patterns. Existing support: Brain search and memory profile; deeper support needs graph/lexical retrieval against old Brain rows.
- Learn means turn a concept or recurring gap into Learn mode. Existing support: quick note Learn action and Learn documents.
- Use means send this exact Brain object into Create, Codex, Claude Code, or a coding-agent prompt. Existing support: Start Create with Brain and export coding prompt.
- UI recommendation: replace broad visible clutter with a compact Brain action dock near the top of the Brain index and object pages. Show 5-7 icon buttons with tooltips and short labels: Capture, Relate, Challenge, Verify, Distill, Revive, Use. Put import/source controls, profile review, and export details behind drawers or details panels.
- Object action recommendation: when a quick note, document, memory, or claim is selected, show only object-relevant actions. For example: Quick note = Save, Relate, Challenge, Learn, Archive; Document = Challenge, Verify, Distill, Use in Create; Memory = Correct, Boost, Forget, Use in Create; Claim = Verify, Challenge, Relate, Revise.
- MVP implementation path: first add a visual action dock that routes existing actions more cleanly; then add one genuinely new Brain-native action, preferably Revive or Challenge, because those make Penny feel like a thinking instrument rather than a prettier archive.
