...
Use one `GMAIL_STAGING_RUN_ID` value for the readiness check, UI preflight, non-destructive smoke, destructive smoke, and full browser evidence JSON. The value must be a safe opaque slug, 3-80 characters, using only letters, numbers, dots, underscores, or hyphens, such as `gmail-staging-2026-05-22-a`. Do not use an email address, URL, account id, token, secret, or raw Nango value. The readiness, UI preflight, and smoke scripts reject unsafe run ids before API calls and omit unsafe values from failure evidence. The standalone readiness, UI preflight, smoke, and full browser evidence verifiers also reject unsafe run ids. The final staging bundle rejects mixed artifacts when this run id is missing, unsafe, or does not match.

Use opaque safe slugs for the evidence scope ids too: `GMAIL_READINESS_USER_ID`, `GMAIL_READINESS_WORKSPACE_ID`, `GMAIL_READINESS_PROJECT_ID`, `GMAIL_READINESS_SPHERE_ID`, and the matching smoke/UI/browser scope fields should contain only letters, numbers, dots, underscores, or hyphens. Do not use the staged Gmail address, a Google account id, a URL, or a token-shaped value for any evidence scope id. The readiness, smoke, and UI preflight scripts reject unsafe evidence scope ids before API calls and omit unsafe values from failure evidence. The standalone readiness, smoke, UI preflight, and browser evidence verifiers, the browser evidence template, and the final staging bundle also reject unsafe scope ids before printing acceptance output.
...
Every readiness run starts by recording an `env.requiredPresence` check. This check is intentionally limited to booleans such as `nangoPublicPresent`, `nangoWebhookSigningKeyPresent`, `nangoGmailIntegrationIdPresent`, `databaseUrlPresent`, `sessionSecretPresent`, `corsOriginsPresent`, and `rateLimitPresent`, plus a closed-world `missingRequirementKeys` array of safe requirement names such as `NANGO_PUBLIC_KEY`, `NANGO_WEBHOOK_SIGNING_KEY`, or `PENNY_SESSION_SECRET`. Failed setup evidence can identify missing staging requirements before any API call without exposing secret values.
...
- `NANGO_SECRET_KEY`, `NANGO_WEBHOOK_SIGNING_KEY`, `NANGO_PUBLIC_KEY`, `NANGO_BASE_URL`, and `NANGO_GMAIL_INTEGRATION_ID` are present, with no placeholder values.
...
UI preflight evidence `checks` are closed-world in the standalone verifier and final bundle verifier. The evidence must include each generated check exactly once (`brain.documents`, `brain.memoryProfile`, `brain.recents`, `google.provider`, and `gmail.status`) and must not include duplicate, legacy, or ad hoc rows. The generated rows must also preserve their route facts: Brain document/profile/recent counts, Google provider surface count and Gmail status, and Gmail status/source/message counts.

Verify the UI preflight evidence before treating it as browser setup proof:

```bash
node --check scripts/verify-gmail-ui-preflight-evidence.mjs
node scripts/verify-gmail-ui-preflight-evidence.mjs tmp/gmail-ui-preflight-evidence.json
```

If the preflight failed before the browser could open, keep the sanitized failure evidence only as setup-debug proof and verify it explicitly:

```bash
node scripts/verify-gmail-ui-preflight-evidence.mjs tmp/gmail-ui-preflight-evidence.json --allow-failure
```
...
The JSON evidence must include `baseUrl`, `userId`, `workspaceId`, `projectId`, `sphereId`, `capturedAt`, `checks`, and at least one sanitized proof artifact in `screenshots`, `notes`, or `proofs`. Full staged browser evidence must include the same safe opaque `stagingRunId` recorded by the readiness, UI preflight, and smoke evidence files; the template generator and standalone browser verifier reject full proof when this run id is missing or unsafe. The `checks` array is closed-world for the verifier mode: include each required browser check at most once, and do not include stale or extra check names. Each proof artifact must include a safe relative `file` or `path` value, plus a non-empty `proves` array listing the check names it supports. Every `proves` entry must match a required browser check for the verifier mode, so pre-OAuth-only evidence cannot claim post-OAuth checks. The full staged browser evidence must include and prove these check names:
...
The smoke evidence `steps` array is closed-world for the verifier mode. Non-destructive evidence may include the optional `connect.preflight` row plus the required status, sync, search, Create, export, and `revoke.delete.skipped` rows; destructive evidence replaces `revoke.delete.skipped` with `revoke` and `deleteSource`; connect-preflight-only evidence may include only `connect.preflight` and `connect.preflightOnly.completed`. Do not add legacy, duplicate, or ad hoc step rows to a file that will be used as acceptance evidence.
...
- `scripts/verify-gmail-ui-preflight-evidence.mjs tmp/gmail-ui-preflight-evidence.json` output for accepted UI preflight evidence, or `--allow-failure` only for sanitized setup-failure evidence that is not being used as final staging proof.
