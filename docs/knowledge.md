...
- Challenge rounds should be rendered as a dedicated card with the live claim context, prior-round trail, why-this-challenge disclosure, explicit response-path selection, and confidence capture, but the actual persistence should still flow through the existing dialectic round route so the audit trail stays unified and the schema-aligned 10-character floor remains enforced.
