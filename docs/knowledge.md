...
- Every API route should validate user-supplied params, query strings, and request bodies with Zod before those values reach persistence or server helpers, and repo IDs should be treated as `cuid()` values rather than UUIDs because the live Prisma models use cuid keys.
- The `/api/maps` route should stay aligned with the rich thought-map capture payload used by the current UI, and `/api/maps/[id]/claims` should support both listing existing claims and creating a new capture from the shared claim-capture form while logging the creation as an applied move.
- The steel-man gate now acts as a natural checkpoint between capture and critique: it uses the existing steel-man persistence route, keeps the 80-character floor, surfaces the quality assessment, and allows an explicit skip path so the flow is not a dead end.
- Challenge rounds should be rendered as a dedicated card with the live claim context, prior-round trail, why-this-challenge disclosure, explicit response-path selection, and confidence capture, and the challenge response routes plus the direct `/api/maps/[id]/dialectic-rounds` POST now trim and enforce the schema-aligned 10-character floor before persistence so the audit trail stays unified.
- `/dashboard` should be a transparent alias of `/app`, not a separate redirect-only route tree, so the dashboard surface stays canonical and the journey does not split by path; the proxy layer should pass that path through instead of redirecting it.
...
- `user`: The Cognitive Fingerprint should be the evolving, evidenced model of the user’s specific reasoning patterns, status changes, and response tendencies, with enough fidelity to surprise the user and be useful back to Penny.
- `user`: The Personal Base Rate Library should derive statistically meaningful per-domain, per-confidence buckets from the user’s resolved prediction history and feed reference-class forcing only when the sample size is actually reliable.
...
- `repo`: The artifact engine is generalized beyond founder brief. `src/lib/artifact-types.ts` defines founder brief, decision memo, investment thesis, research proposal, risk register, personal decision audit, and hypothesis brief as first-class artifact types, and the builder can render a section-level diff against the previous version.
...
- `repo`: `src/app/page.tsx` now redirects signed-in users to `/dashboard`, keeps the landing copy aligned with the wiki-first product direction, and uses the `/auth/sign-in` and `/auth/sign-up` entry points for the public CTA flow.
- `repo`: `src/components/penny/nav.tsx` now provides the shared signed-in navigation for `/dashboard` and `/app`, with map selection, quick capture, new map, settings, and sign-out controls reused by both shells.
...
- `repo`: `src/components/penny/shape-detail.tsx` now exposes full shape derivation explainability, including contributing moves, confirmation/disconfirmation weights, thresholds, counterfactual retirement conditions, alternative shapes, and a falsification-condition editor.
- `repo`: `src/components/penny/meta-cognition-prompt.tsx` and `src/lib/meta-cognition.ts` now surface sparse meta-cognition prompts from the active lens, with trigger evidence, cooldown-aware logging, disagreement capture, and session-context logging through `POST /api/maps/[id]/meta-cognition`.
