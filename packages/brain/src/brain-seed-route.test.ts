...
test("POST /brain/seed persists the seed and returns a UI-ready payload", async () => {
  ...
  assert.match(payload.data.learningPlan.groups[0]?.subgroups[0]?.oneLineGoal ?? "", /subsection/i);
  assert.equal(payload.data.learningPlan.groups[0]?.subgroups[0]?.teachingSections.length, 3);
