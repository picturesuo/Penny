...
## Later
- [x] Build the learning feature so Penny can recommend the best next thing to learn from the current weakest branch.
- [x] Connect learning recommendations to validation tasks, not generic reading advice.
- [x] Close the loop from capture -> stress test -> learn -> act.
...
- [x] The learning lane could ship from existing weakest-node, missing-kind, recommended-move, and founder-brief data without waiting for a separate learning backend.
