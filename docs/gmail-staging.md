...
Use opaque safe slugs for the evidence scope ids too: `GMAIL_READINESS_USER_ID`, `GMAIL_READINESS_WORKSPACE_ID`, `GMAIL_READINESS_PROJECT_ID`, `GMAIL_READINESS_SPHERE_ID`, and the matching smoke/UI/browser scope fields should contain only letters, numbers, dots, underscores, or hyphens. Do not use the staged Gmail address, a Google account id, a URL, or a token-shaped value for any evidence scope id. The readiness, smoke, and UI preflight scripts reject unsafe evidence scope ids before API calls and omit unsafe values from failure evidence. The browser evidence template, browser evidence verifier, and final staging bundle also reject unsafe scope ids before printing acceptance output.
