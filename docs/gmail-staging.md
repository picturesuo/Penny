...
Use opaque safe slugs for the evidence scope ids too: `GMAIL_READINESS_USER_ID`, `GMAIL_READINESS_WORKSPACE_ID`, `GMAIL_READINESS_PROJECT_ID`, `GMAIL_READINESS_SPHERE_ID`, and the matching smoke/UI/browser scope fields should contain only letters, numbers, dots, underscores, or hyphens. Do not use the staged Gmail address, a Google account id, a URL, or a token-shaped value for any evidence scope id. The final staging bundle rejects unsafe scope ids before printing an acceptance summary.
