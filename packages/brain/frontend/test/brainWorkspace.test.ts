...
test("BrainMemoryPanel renders imported sources, profile summary, and recent memory nodes", () => {
  ...
  assert.match(markup, /Email fixture, not live Gmail/);
  assert.match(markup, /LinkedIn-style context, not live LinkedIn/);
  assert.match(markup, /Manual messages context for demo, not live WhatsApp, iMessage, or SMS/);
  assert.match(markup, /Founder notes, manual\/private/);
