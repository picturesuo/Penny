...
| Brain | Start Create With This Brain | Open Create carrying actual Brain context. | Works; Create opens with Brain context and a generated rough idea from `createPromptFromBrainProfile`. Recent memory cards also expose a compact `Use in Create` action that seeds Create from that specific memory while carrying the full Brain profile. | `handleStartCreateWithBrain` sets both `createBrainProfile` and `createInitialSeedText`; frontend tests cover both whole-profile and specific-memory seed prompts plus the memory-card action. | Later: let users pick a specific Brain document as the seed if document-level Create becomes a real workflow. |
