...
The JSON evidence must include `baseUrl`, `userId`, `workspaceId`, `projectId`, `sphereId`, `capturedAt`, `checks`, and at least one sanitized proof artifact in `screenshots`, `notes`, or `proofs`. Full staged browser evidence must include the same safe opaque `stagingRunId` recorded by the readiness, UI preflight, and smoke evidence files; the template generator and standalone browser verifier reject full proof when this run id is missing or unsafe. Each proof artifact must include a safe relative `file` or `path` value, plus a non-empty `proves` array listing the check names it supports. Every `proves` entry must match a required browser check for the verifier mode, so pre-OAuth-only evidence cannot claim post-OAuth checks. The full staged browser evidence must include and prove these check names:
