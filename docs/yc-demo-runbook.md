...
4. Confirm five Create cards are visible: `Personal`, `Practical`, `Valuable`, `Critical`, `Weird`.
5. Open `Details` on `Personal` and point at `Memories used` and `Sources used`.
6. Select `Personal`, `Valuable`, and `Critical`.
7. In `Comment`, type: `Keep this founder/builder path: memory-native workbench, human judgment, and buildable specs before coding agents.`
8. Click `Update artifact`.
9. Point at the artifact update and verification panel.
10. Click `Learn this`.
11. In Learn, show `Explain simply`, `Show worked example`, and `Show how this applies to my artifact`.
12. Click `Back to Create`.
13. Confirm the selected cards and comment are preserved.
14. Click `Export prompt`.
15. Show the exported prompt includes `Personal Context Used`, selected option history, and source/memory evidence.
...
**2:20-3:00 - Export**
I export the prompt. The artifact now carries the rough idea, selected option history, source/memory evidence, constraints, and implementation requirements. The export is what I would hand to Codex or another coding agent.
