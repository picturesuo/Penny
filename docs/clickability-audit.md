...
- Browser e2e: `yc-recording.spec.cjs`, `brain-first.spec.cjs`, and `learn-understanding-tour.spec.cjs` passed together, 3 tests in 7.1s; Brain-first now asserts quick-note save creates private Brain memory, Learn asserts the Source/Map/Teach/Use/Check tour, YC reloads inside the Create Learn bridge, and Brain/Create export paths assert visible `Copy prompt` and `Download .md` actions.
...
| Brain | Review Brain Profile | Mark profile review complete. | Works; Brain shows a profile review card and the first-run flow only marks review done after `Profile looks right`. | `POST /api/brain/memory/profile/review` now records explicit `profile_reviewed` Brain development events, the profile response returns latest review plus newest-first `profileReviewHistory`, and the UI shows a compact expandable review history after reload. | Keep history compact; only expand into comparison tools if profile revisions become a real workflow. |
...
| Brain | Boost memory | Increase memory importance. | Works; boosted/high-confidence nodes get a visible state label, the update notice names the reviewed memory, and Create evidence now shows compact confidence/evidence/rank-effect labels. | Brain boost raises confidence into the boosted threshold; retrieval/profile/ranker paths preserve `rankEffect`, and Create appends human-weighted memories into option evidence. | Keep raw score math hidden; show only confidence/evidence/rank-effect labels. |
...
| Brain | Start Create With This Brain | Open Create carrying actual Brain context. | Works; Create opens with Brain context and a generated rough idea from `createPromptFromBrainProfile`. Recent memory cards also expose a compact `Use in Create` action that seeds Create from that specific memory while carrying the full Brain profile. Saved Brain documents use `Rework in Create` to boot a fresh Create run from the document's title, original idea, main claim, recommendations, and next actions. | `handleStartCreateWithBrain` sets both `createBrainProfile` and `createInitialSeedText`; frontend tests cover whole-profile, specific-memory, and saved-document seed prompts plus the memory-card action. | Keep. If document Create becomes primary, add source chips that show which document sections seeded the run. |
...
| Create | Brain pill/button | Return to Brain from Create. | Works; the Create-specific return control has `aria-label="Open Brain from Create"` and frontend coverage. | `CreateWorkspace` renders a scoped Brain button with a unique accessible name. | Keep the accessible name stable for smoke tests. |
...
| Create | Rough idea textarea | Editable idea input. | Works and restores after refresh. | `draftText` is included in the Create workspace draft stored in local storage. | Keep. Move to server-backed Create sessions only if cross-device continuity becomes part of the MVP. |
...
| Learn | Back to Create | Return with prompt/sources/selections/comment/artifact/evidence preserved. | Works after a reload inside Learn; the app restores the Learn panel from the Create boot record, then Back to Create restores the selected cards, founder comment, artifact, and open evidence drawer. | `rememberCreateWorkspaceView` stores `activeMode` and `learnFocusNode`; `CreateWorkspace` stores selected/rejected/comment/artifact/export/details state in local draft storage. | Keep. Later move this draft into server-backed Create sessions if cross-device continuity becomes important. |
